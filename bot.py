...
async def on_member_join(member):
    ...
    if random.randint(1, 2) == 2:
        sendGoogledImageMessage(member)

# call google api to find image with users name and send it to chat
async def sendGoogledImageMessage(member):
    _search_params = {
    'q': member.name,
    'num': 1,
    'safe': 'medium',
...
async def on_message(message):
    ...
    if message.content.lower() == "hey tf bot. who am i?":
        await sendGoogledImageMessage(message.author)
